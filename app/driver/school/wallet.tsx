// app/driver/school/wallet.tsx
// FIX: '@/constants/themes' → '@/constants/theme'
// FIX: Same fixes as meter wallet — totalEarned sum, modal keyboard handling,
//      bottom-sheet withdrawal modal, amount input filter.
// NOTE: AppwriteService.getSchoolDriverWallet / createSchoolDriverWallet /
//       getSchoolDriverTransactions / requestSchoolDriverWithdrawal must be
//       defined by the developer following the existing AppwriteService pattern.
...
export default function SchoolDriverWalletScreen() {
  const { user } = useUser();
  ...
  const loadData = async () => {
    ...
    try {
      const drivers = await databases.listDocuments(
        DATABASE_ID, COLLECTIONS.SCHOOL_DRIVERS, [Query.equal('userId', user.id)],
      );
      if (!drivers.documents.length) throw new Error('Driver not found');
      const driver = drivers.documents[0];
      setDriverId(driver.$id);

      let w = await AppwriteService.getSchoolDriverWallet(driver.$id);
      if (!w) w = await AppwriteService.createSchoolDriverWallet(driver.$id);
      setWallet(w);

      const txns = await AppwriteService.getSchoolDriverTransactions(driver.$id, 50);
  ...
  const handleWithdraw = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await AppwriteService.requestSchoolDriverWithdrawal(driverId!, amount, bankAccount);
      if (result.success) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* Action */}
        <TouchableOpacity style={styles.withdrawBtn} onPress={() => setWithdrawVisible(true)} activeOpacity={0.85}>
        ...
        {transactions.length === 0 ? (
          <EmptyState icon="💰" title="No transactions yet" subtitle="Your earnings will appear here once parents have paid." />
        ) : (
          transactions.map(tx => (
...
const styles = StyleSheet.create({
  ...
  withdrawBtn:    { backgroundColor: COLORS.primary, paddingVertical: 14, borderRadius: RADIUS.lg, alignItems: 'center', marginBottom: SPACING.lg, ...SHADOWS.sm },
