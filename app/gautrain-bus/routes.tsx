...
export default function BusRoutesScreen() {
  const busRoutes = getRoutesByType(3); // 3 = bus

  const renderItem = ({ item }: any) => (
    <TouchableOpacity
      style={styles.routeItem}
      onPress={() => router.push({ pathname: '/gautrain-bus/route-detail', params: { routeId: item.route_id } })}
    >
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Gautrain Bus Routes</Text>
        <View style={{ width: 50 }} />
      </View>
      <FlatList
        data={busRoutes}
